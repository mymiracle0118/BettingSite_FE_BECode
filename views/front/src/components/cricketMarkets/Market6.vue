<template>
    <div class="market-6">
        <div class="bet-table">
            ...
            <div class="bet-table-body collapse show container-fluid container-fluid-5" :id="`market${index}`"
                ...
                <div class="row row5">
                    <template v-for="(section, key1) in newMarketSections">
                        <div class=" col-12 col-md-6" :key="`web${key1}`">
                            <div class="fancy-tripple">
                                ...
                                <template v-if="filterBy(section.odds, 'back1', 'oname').length">
                                    <div class="bet-table-row" :data-title="section.gstatus"
                                        ...
                                        <div class="nation-name d-none-mobile">
                                            <p @click="getBookData(marketDataNew.mid, section.sid, $parent.gameId, marketDataNew.gtype)">
                                                {{ section.nat }}
                                            </p>
                                            <p class="mb-0" v-fancybook="{mid: marketDataNew.mid, sid: section.sid}">0</p>
                                        </div>
                                        <template v-for="(oddsData, key2) in filterBy(section.odds, 'lay1', 'oname')">
                                            <div class="bl-box lay" :key="`layodds${key2}`" :class="[{'no-val':oddsData.odds == 0},
                                                ...
                                                <span class="d-block odds" v-odds="{odds: oddsData.odds}">
                                                    {{ oddsData.odds }}
                                                </span>
                                        ...
                                        <template v-for="(oddsData, key2) in filterBy(section.odds, 'back1', 'oname')">
                                            <div class="bl-box back" :key="`backodds${key2}`" :class="[{'no-val':oddsData.odds == 0},
                                                ...
                                                <span class="d-block" v-if="oddsData.size != 0" v-kval="{size: oddsData.size}">
                                                    {{ oddsData.size }}
                                                </span>
                                            </div>
                                        </template>
                                ...
                                <template v-if="filterBy(section.odds, 'back2', 'oname').length">
                                    <div class="bet-table-row" :data-title="section.gstatus"
                                        ...
                                        <template v-for="(oddsData, key2) in filterBy(section.odds, 'lay2', 'oname')">
                                            <div class="bl-box lay" :key="`layodds${key2}`" :class="[{'no-val':oddsData.odds == 0},
                                                getClass(oddsData.odds, 'lay', oddsData.tno, key1 )]"
                                                @click="$parent.setBetData(oddsData.odds, oddsData.otype, section.sid, marketDataNew.mid, marketDataNew.mname, section.nat, marketDataNew.gtype, oddsData.size, 0, newMarketSections)">
                                                <span class="d-block odds" v-odds="{odds: oddsData.odds}">
                                        ...
                                        <template v-for="(oddsData, key2) in filterBy(section.odds, 'back2', 'oname')">
                                            <div class="bl-box back" :key="`backodds${key2}`" :class="[{'no-val':oddsData.odds == 0},
                                                getClass(oddsData.odds, 'back', oddsData.tno, key1 )]"
                                                @click="$parent.setBetData(oddsData.odds, oddsData.otype, section.sid, marketDataNew.mid, marketDataNew.mname, section.nat, marketDataNew.gtype, oddsData.size, 0, newMarketSections)">
                                                <span class="d-block odds" v-odds="{odds: oddsData.odds}">
                                ...
                                <template v-if="filterBy(section.odds, 'back3', 'oname').length">
                                    <div class="bet-table-row" :data-title="section.gstatus"
                                        ...
                                        <template v-for="(oddsData, key2) in filterBy(section.odds, 'lay3', 'oname')">
                                            <div class="bl-box lay" :key="`layodds${key2}`" :class="[{'no-val':oddsData.odds == 0},
                                                getClass(oddsData.odds, 'lay', oddsData.tno, key1 )]"
                                                @click="$parent.setBetData(oddsData.odds, oddsData.otype, section.sid, marketDataNew.mid, marketDataNew.mname, section.nat, marketDataNew.gtype, oddsData.size, 0, newMarketSections)">
                                                <span class="d-block odds" v-odds="{odds: oddsData.odds}">
                                        ...
                                        <template v-for="(oddsData, key2) in filterBy(section.odds, 'back3', 'oname')">
                                            <div class="bl-box back" :key="`backodds${key2}`" :class="[{'no-val':oddsData.odds == 0},
                                                getClass(oddsData.odds, 'back', oddsData.tno, key1 )]"
                                                @click="$parent.setBetData(oddsData.odds, oddsData.otype, section.sid, marketDataNew.mid, marketDataNew.mname, section.nat, marketDataNew.gtype, oddsData.size, 0, newMarketSections)">
                                                <span class="d-block odds" v-odds="{odds: oddsData.odds}">
        ...
        methods: {
            ...
            getClass(oddsNew, otype, oddsTno, sectionKey) {
                ...
                if (this.oldMarketSections && this.oldMarketSections[sectionKey]) {
                    let oddsObj = _.first(this.find(this.filterBy(_.get(this.oldMarketSections[sectionKey], 'odds'),
                        otype,
                        'otype'), oddsTno, 'tno'))

<template>
  <div class="detail-page-container" :class="{ 'loader-section-parent' : gameData.new && !gameData.new.length }">
    ...
    <b-modal ref="placebet-modal" hide-footer title="BET SLIP" @hide="$store.commit('resetBetData')"
      ...
      <ValidationObserver tag="div" v-slot="{ invalid }">
        <template v-if="$route.name == 'gameDetail' && $store.state.bet && $store.state.bet != null">
    ...
    <b-modal id="betSodaModal" hide-footer title="MY BETS" class="marketModal">
      ...
      <template v-if="$route.name == 'gameDetail' && $store.state.betSoda != null">

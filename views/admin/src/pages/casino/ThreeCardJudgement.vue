            ...
            <div class="casino-container">
                <div class="casino-table threecardj">
                    ...
                    <div class="casino-detail"  v-if="casinoData && casinoData.mid">
                        ...
                        <div class="threecardj-container">
                            <template v-for="(data, key) in mainData">
                            <div class="threecardj-bl-box" :class="{back: key == 0, lay: key == 1}">
                                <div class="threecardj-title">
                                    <b>{{ data.nat }}</b>
                                    <span class="casino-book" v-casinofancybook="{sid: data.sid}">0</span>
                                </div>
                                <div class="threecardj-cards" :class="{'suspended': data.gstatus != 'OPEN'}">
                                    <div class="threecardj-odds text-center">
                                        <b>{{ data.b }}</b>
                                    </div>
                                    <div class="casino-cards text-center mt-1">
                                        <div class="casino-card-item" v-for="(card, key) in 13">
                                            <div class="card-image ">
                                                <Card v-if="card == 1" :card="'A'" />
                                                <Card v-else-if="card == 11" :card="'J'" />
                                                <Card v-else-if="card == 12" :card="'Q'" />
                                                <Card v-else-if="card == 13" :card="'K'" />
                                                <Card v-else :card="card" />
                                            </div>
                                        </div>
                                    </div>
                                    <div class="casino-min-max text-right">
                                        <span>{{ casinoData.remark.split("|")[key] }}</span>
                                    </div>
                                </div>
                            </div>
                            </template>
                        ...
                        <div class="text-right casino-min-max">R:<span v-kval="{size: mainData[0].min}"></span>-<span
                                                        v-kval="{size: mainData[0].max}"></span></div>  

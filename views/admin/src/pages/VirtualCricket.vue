<template>
  <div  class="detail-page-container"  v-if="gameDetail && gameDetail.etid"> 
          <div class="center-main-container">
              <div class="center-content">
                  <div class="casino-table five-cricket detail-page-container">
                     <div class="game-header" :class="`sport${gameDetail.etid}`">
                      ...
                      <span class="float-right game-header-date" v-format-datetime="{date: gameDetail.stime}"></span>
                    </div>
                    <div class="container-fluid container-fluid-5">
                      <div class="row row5">
                        <div class="col-xl-2 banner d-flex align-items-center" >
                          <template v-if="gameDetail.iplay == true">
                        ...
                        <div class="col-xl-10">
                          ...
                          <div class="video-box-container">
                              <div class="video-box" v-if="gameDetail.iplay == true && tvUrl != ''">
                                  <iframe :src="tvUrl">
                                  </iframe>
                              </div>
                              <div v-else class="five-video-baner">
                                  <img v-if="gtype != ''" :src="imgPath+'front/img/'+gtype+'-bg.jpg'">
                          ...
                          <div class="casino-video-right-icons">
                              ...
                              <div class="casino-video-rules-icon" @click="reloadPage()">
                                  <i class="fas fa-sync-alt" :class="{'fa-spin': isLoading == true}"></i>
                              </div>
                              <b-modal id="rules-modal" modal-class="casino-rules" title="Rules" hide-footer>
                              <img :src="`${imgPath}front/img/casino-rules/${gtype}.jpg`" class="img-fluid">
                              </b-modal>
                  ...
                  <div class="banner scorestats mb-1" v-if="gameDetail && gameDetail.gtv != 0 && gameDetail.gtv != undefined">
                    <template v-if="gameDetail.etid">
                      <img :src="`${imgPath}front/img/events-banner/${gameDetail.etid}.png`" class="img-fluid">
                    </template>
                    <iframe :src="`https://royalexch999.com/scoreanimation/${gameDetail.gtv}/1234`" frameborder="0"></iframe>
                  </div>
                  <div class="more-market text-right" v-if="gameDetail != null && gameDetail.etid != '4'">
                  ...
                  <div class="market-container">
                      <template v-for="(data, key) in orderBy(gameData.new, 'sno')">
                        ...
                        <template v-if="data.dtype == 1">
                          <Market1 :key="key" :index="key" :marketDataNew="data" :marketDataOld="orderBy(gameData.old, 'sno')[key]" />
                        </template>
                        ...
                        <template v-if="data.dtype == 3">
                          <Market3 :key="key" :index="key" :marketDataNew="data" :marketDataOld="orderBy(gameData.old, 'sno')[key]" />
                        </template>
                        ...
                        <template v-if="data.dtype == 5">
                          <Market5 :key="key" :index="key" :marketDataNew="data" :marketDataOld="orderBy(gameData.old, 'sno')[key]" />
                        </template>
                        ...
                        <template v-if="data.dtype == 7">
                          <Market7 :key="key" :index="key" :marketDataNew="data" :marketDataOld="orderBy(gameData.old, 'sno')[key]" />
                        </template>

                        <template v-if="data.dtype == 8">
                          <Market8 :key="key" :index="key" :marketDataNew="data" :marketDataOld="orderBy(gameData.old, 'sno')[key]" />
                        </template>

                        <template v-if="data.dtype == 9">
                          <Market9 :key="key" :index="key" :marketDataNew="data" :marketDataOld="orderBy(gameData.old, 'sno')[key]" />
                        </template>

                        <template v-if="data.dtype == 10">
                          <Market10 :key="key" :index="key" :marketDataNew="data" :marketDataOld="orderBy(gameData.old, 'sno')[key]" />
                        </template>

                        <template v-if="data.dtype == 11">
                          <Market11 :key="key" :index="key" :marketDataNew="data" :marketDataOld="orderBy(gameData.old, 'sno')[key]" />
                        </template>
                      </template>
                      </template>
              ...
              <div class="right-sidebar" :data-simplebar="!$store.getters.isMobile">
                <div class="card m-b-10">
                    ...
                    <div class="card-header pointer" data-toggle="collapse" data-target=".video-tv" aria-expanded="true" @click="showTv = !showTv" >
                        <h6 class="card-title"><a href="javascript:void(0)" title=""><img src="/static/admin/img/arrow-down.svg" class="mr-1"></a> Live Match</h6>
                    </div>
                    <div class="video-tv collapse" >
                        <iframe  v-if="showTv" :src="`/admin/pages/mediaplayergame/${gameId}/${$store.getters.ipAddress}`">
                        </iframe>
                    </div>
                    </template>
                </div>
          ...
          <table class="table">
              ...
              <tbody>
                  <template v-if="marketList != null && marketList.length > 0">
                    ...
                    <tr>
                        <td><a href="javascript:void(0)" @click="setMarketId(market.mid)">{{ market.mname}}</a></td>
      ...
      <b-modal ref="userLockModal" hide-footer title="Bet Lock" class="">
        <div class="row">
          ...
          <div class="col-4">
            <div class="form-group">
              <input type="password" name="ulockmpass" v-model="userLock.mpass" value="" class="form-control" placeholder="Transaction Code">
            </div>
          </div>
        </div>
        <div class="table-responsive">
          <table class="table table-bordered">
              <thead>
                  <tr>
                      <th><b-form-checkbox v-model="userLock.lockAll" value="1" @change="lockUserAll($event)" ></b-form-checkbox></th>
                      <th>All Account</th>
                  </tr>
              </thead>
              <tbody>
                  <template v-if="userLock.userList != null && userLock.userList.length > 0">
                    ...
                    <tr>
                        <td><b-form-checkbox  name="locku" v-model="userLock.selected" :value="user.uguid" @change="lockUser(user, $event)" :ref="'user'+user.uguid"></b-form-checkbox></td>
                        <td>{{ user.uname}}</td>
                    </tr>
                    </template>
                  </template>
                ...
                <thead>
                  <tr>
                    ...
                    <template v-for="(val, key) in allBookTreeData[0]">
                      <th
                        class="text-right"
                        v-if="key!='children' && key!='levelno' && key!='userid' && key!='subid' && key!='Username' "
                      >{{ key }}</th>
                    </template>
                  </tr>
                </thead>
  ...
  methods: {
    ...
    getGameDetail() {
      ServiceApi.callApi("game/detail", {
          ...
          s_token: SECURE_POLLING == 2 ? hiddenPoller.decrypt(
        ...
        .then(response => {
          if (response.status == 200) {
            ...
            this.gtype = _.get(this.gameDetail, 'gtype')
            this.m = _.get(this.gameDetail, 'm')
            
            this.mid = this.gameDetail.mid;
            this.getTvUrl();
            //this.tvUrl = '/admin/pages/mediaplayercasino/'+this.gtype+'?ip='+IP_ADDRESS;
    ...
    getTvUrl() {
        ...
        .then(response => {
            if (response.status == 200) {
                this.tvUrl = '/admin/pages/mediaplayercasino/' + this.gtype + '/' + response.data.token + '?ip=' + IP_ADDRESS;
            } else {
                this.tvUrl = ''
            }
        })
    ...
    getCricketvData() {
      ServiceApi.callApi("game/getcricketvdata", {
        ...
        .then(response => {
          if (response.status == 200) {
            ...
            let responseData = response.data.t1;
            if(responseData.card != ""){
              this.cards = _.get(responseData, 'card').split('|')
            }else{
              this.cards = []
            }
            this.time.total = _.get(responseData, 'ft')
            this.time.current = _.get(responseData, 'lt')
    ...
    getBookData(mid, gtype) {
      ...
      .then(response => {
        if (response.status == 200) {
          ...
          this.allBookTreeData = this.buildAllBookTree(this.allbook, this.allbook[0].subid);

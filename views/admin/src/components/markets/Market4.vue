    ...
    <div class="market-4">
        <div class="bet-table">
            ...
            <div class="bet-table-body collapse show" :id="`market${index}`"
                ...
                <div class="bet-table-row bet-table-row-top" v-if="!$store.getters.isMobile">
                    <div class="text-right nation-name">
                        ...
                        <span class="max-bet" v-else>Max:<span v-kval="{size: marketDataNew.maxb}"></span></span> -->
                            ...
                            <span class="max-bet" v-if="marketDataNew.gtype == 'match1'">Min:<span v-kval="{size: marketDataNew.min}"></span> Max:<span v-kval="{size: marketDataNew.max}"></span></span>
                            <span class="max-bet" v-else-if="marketDataNew.gtype == 'match'"><span v-kval="{ size: marketDataNew.iplay ? marketDataNew.maxb : marketDataNew.umaxbof }" v-b-tooltip.hover :title="`Max : ${kval(marketDataNew.iplay ? marketDataNew.maxb : marketDataNew.umaxbof)}`"></span></span>
                ...
                <div class="text-right d-inline-block w-100 pr-2" v-else>
                    <span class="max-bet" v-if="marketDataNew.gtype == 'match1'">Min:<span v-kval="{size: marketDataNew.min}"></span> Max:<span v-kval="{size: marketDataNew.max}"></span></span>
                            <span class="max-bet" v-else-if="marketDataNew.gtype == 'match'">Max: <span v-kval="{ size: marketDataNew.iplay ? marketDataNew.maxb : marketDataNew.umaxbof }" v-b-tooltip.hover :title="`Max : ${kval(marketDataNew.iplay ? marketDataNew.maxb : marketDataNew.umaxbof)}`"></span></span>
                            <span class="max-bet" v-else>Max:<span v-kval="{size: marketDataNew.iplay ? marketDataNew.maxb : marketDataNew.umaxbof}"></span></span>
                </div>
        ...
        <small class="remark" v-if="marketDataNew.rem != ''">{{ marketDataNew.rem }}</small>
    </div>
</template>
